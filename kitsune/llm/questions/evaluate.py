...
from kitsune.llm.questions.classifiers import classify_question, ModerationAction
...
class ClassifyQuestionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.sync_data = TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to syncing data between Firefox"
                    " browsers across platforms and devices. This topic can apply to data"
                    " like passwords, browser sessions, tabs, bookmarks, add-ons, and settings."
                ),
            ),
            products=[firefox, mobile, ios, ma],
        )
        ...
        t5_2 = TopicFactory(
            ...
            metadata=dict(
                description=(
                    ...
                    " products, including any related settings."
                ),
            ),
            products=[firefox, mobile, ios],
        )
        self.pip = TopicFactory(
            title="PiP",
            ...
            metadata=dict(
                description=(
                    ...
                    " within Firefox, including any related settings like enabling PiP video"
                    " controls."
                ),
            ),
            products=[firefox, mobile, ios],
        ...
        self.browse_history = TopicFactory(
            title="History",
            ...
            metadata=dict(
                description=(
                    ...
                    " browsing history, including any related settings like whether or not to"
                    " remember browser history, clearing browser history, etc."
                ),
            ),
            products=[firefox, mobile, ios],
        )
        TopicFactory(
            ...
            parent=self.browse_history,
            metadata=dict(
                description=(
                    "Content, questions, or issues about cookies/cache as they relate to the"
                    " browser history, including any related settings like whether or not to"
                    " delete cookies when closing the browser, etc."
                ),
            ),
            products=[firefox, mobile, ios],
        )
        ...
        TopicFactory(
            ...
            metadata=dict(
                description=(
                    ...
                    " Mozilla's products, including any related settings. For example, the"
                    " dashboard on the homepage of Firefox that displays a user's shortcuts,"
        ...
        TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to Firefox New Tab and any related"
                    " settings like what is displayed in a new tab, etc."
                ),
            ),
            products=[firefox, mobile, ios],
        )
        ...
        self.tabs = TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to managing browser tabs, including"
                    " any related settings like tab layout (horizontal or vertical), whether to"
                    " open links in tabs rather than windows, whether to ask before quitting"
                    " tabs, etc."
                ),
            ),
        ...
        t6 = TopicFactory(
            ...
            metadata=dict(
                description=(
                    ...
                    " content, including any related related settings like what folder to save"
                    " downloaded files to, and whether to always ask where to save files to."
                ),
            ),
        ...
        t6_1 = TopicFactory(
            ...
            metadata=dict(
                description=(
                    ...
                    " handled and where they’re saved, including any related settings."
                ),
            ),
        ...
        TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to saving usernames and"
                    " passwords, or automatic input of usernames and passwords into"
                    " sign in fields using the autofill feature. Note that this topic"
                    " is unrelated to Mozilla passwords."
                ),
            ),
            products=[firefox, mobile, ios, vpn],
        )
        ...
        t13_1 = TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to enhancing product functionality"
                    " with add-ons, extensions, and add-on themes. This does not apply to themes"
                    " that are built into the product, like light or dark mode."
                ),
            ),
        ...
        TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, requests, or issues related to add-on themes. This"
                    " does not apply to themes that are built into the product, like light"
                    " or dark mode."
                ),
            ),
        ...
        TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to automatically filling addresses"
                    " and payment cards into form fields."
                ),
            ),
        ...
        self.browser_appearance = TopicFactory(
            ...
            metadata=dict(
                description=(
                    ...
                    " appearance, like customizing the default font, showing the sidebar, or"
        ...
        t13_4 = TopicFactory(
            ...
            metadata=dict(
                description=(
                    "Content, questions, or issues related to importing and exporting product,"
                    " account, or profile settings, including how to import settings from a"
                    " non-Firefox browser into a Firefox browser."
                ),
            ),
    ...
    def test_firefox_spam(self):
        for content in [
            ...
            with self.subTest(content):
                question = QuestionFactory(product=self.firefox, content=content)

                result = classify_question(question)

                self.assertIn("action", result)
                self.assertIn("spam_result", result)
                self.assertIn("topic_result", result)

                spam_result = result["spam_result"]
                self.assertIn("is_spam", spam_result)
                self.assertIn("confidence", spam_result)
                self.assertIn("reason", spam_result)

                self.assertEqual(
                    result["action"],
                    ModerationAction.SPAM,
                    f"Not spam! spam_result:\n{result.get('spam_result')}",
                )

                self.assertFalse(result["topic_result"])

    def test_firefox_topics(self):
        for content, expected in [
            (
                "How do I make my tabs vertical rather than horizontal?",
                ("Tabs", "Browser appearance"),
            ),
            (
                "I just bought a new computer and installed Firefox, but I don't know how"
                " to get my old bookmarks and other settings. Help!",
                ("Backup, recovery, and sync", "Sync data", "Sync configuration"),
            ),
            (
            ...
            (
                "Does Firefox protect me from companies tracking my browsing?",
                ("Privacy and security", "Tracking protection"),
            ),
            (
                "Does Firefox block tracking cookies?",
                ("Privacy and security", "Tracking proteciton", "Cookies as trackers"),
            ),
            (
                "The Address Bar apparently honors abbreviations of * (for search within"
                " Bookmarks only) and ^ (for search within History only). Just about every"
                " reference, in Firefox Help, seems to equate the Search Bar with the Address"
                " Bar. So, I would expect that the Search Bar would honor said abbreviations."
                " But, it does not. Isn't it supposed to?\n\nConfiguration:\nFirefox Desktop,"
                " v138.0.4",
                ("Search, tag, and share", "Search"),
            ),
            (
                "Google is set as the default search engine but I would like to change it to Bing",
                ("Settings", "Customization", "Search, tag, and share", "Search"),
            ),
        ]:
            with self.subTest(content):
                question = QuestionFactory(product=self.firefox, content=content)
                ...
                self.assertIn("topic", topic_result)
                self.assertIn("reason", topic_result)
                self.assertIn(
                    topic_result["topic"],
                    expected,
                    f"Unexpected topic: topic_result:\n{topic_result}",
                )

    def test_thunderbird_for_android_topics(self):
        for content, expected in [
            (
                'BTW: The selection box "Which topic best describes your question?" should be'
                ' added another category: "I want to file a bug report".\n\nThis is a bug'
                " report concerns Thunderbird on Android Version 9.0 running on a Samsung"
                " Note-3 running with Android 5.0:\n\nWhen I enter an email address, it often"
                " happens that TB crashes when the second letter of the adressse's name is"
                " entered.\n\nHowever, If I go to the address book and select the same user by"
                " his email address contained in it, I can write an email address to him, which"
                " I could not do the other way by directly entering the adressee's email adress"
                " into the appropriate field.",
                ("App crash",),
            ),
        ]:
            with self.subTest(content):
                question = QuestionFactory(product=self.tba, content=content)
