...
def get_pinned_articles(user=None, product=None, locale=settings.WIKI_DEFAULT_LANGUAGE):
    ...
    if locale == settings.WIKI_DEFAULT_LANGUAGE:
        qs = Document.objects.visible(user, locale=locale, id__in=pinned_articles)
    else:
        qs = Document.objects.visible(user, locale=locale).filter(
            Q(id__in=pinned_articles) | Q(parent__in=pinned_articles)
        )
...
def get_featured_articles(
    user=None, product=None, topics=None, locale=settings.WIKI_DEFAULT_LANGUAGE, limit=4
) -> list[Document]:
    """
    ...
    """
    pinned_articles = list(get_pinned_articles(user=user, product=product, locale=locale)[:limit])
