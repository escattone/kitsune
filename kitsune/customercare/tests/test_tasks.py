...
class PostReplyToZendeskTests(TestCase):
    ...
    def setUp(self):
        ...
        self.ticket.save(update_fields=["pending_comment"])
    ...
    def test_happy_path_clears_pending_and_adds_to_comments(self, mock_client_cls):
        ...
        self.ticket.refresh_from_db()
        self.assertEqual({}, self.ticket.pending_comment)
        self.assertEqual(1, len(self.ticket.comments))
    ...
    def test_permanent_api_error_marks_failed_and_reraises(self, mock_client_cls):
        ...
        self.ticket.refresh_from_db()
        self.assertEqual([], self.ticket.comments)
        pending = self.ticket.pending_comment
    ...
    @patch("kitsune.customercare.tasks.sync_ticket_from_zendesk")
    @patch("kitsune.customercare.tasks.ZendeskClient")
    def test_verify_match_on_reattempt_clears_pending_without_posting(
        self, mock_client_cls, mock_sync
    ):
        # Re-attempt: attempts > 0, last_attempted_at set. Verify finds a matching
        # comment in the refreshed mirror → drop pending without posting.
        self.ticket.pending_comment["attempts"] = 1
        self.ticket.pending_comment["last_attempted_at"] = "2026-05-14T00:00:00Z"
        self.ticket.save(update_fields=["pending_comment"])

        def fake_sync(ticket):
            ticket.comments = [
                {
                    "id": 99,
                    "body": "hello",
                    "created_at": "2026-05-14T00:00:30Z",
                    "public": True,
                    "author": {"name": "self", "id": 555},
                }
            ]
            ticket.save(update_fields=["comments"])
        ...
        self.assertEqual({}, self.ticket.pending_comment)
        self.assertEqual(1, len(self.ticket.comments))
        self.assertEqual(99, self.ticket.comments[0]["id"])

    @patch("kitsune.customercare.tasks.sync_ticket_from_zendesk")
    @patch("kitsune.customercare.tasks.ZendeskClient")
    def test_verify_no_match_on_reattempt_proceeds_to_post(self, mock_client_cls, mock_sync):
        self.ticket.pending_comment["attempts"] = 1
        self.ticket.pending_comment["last_attempted_at"] = "2026-05-14T00:00:00Z"
        self.ticket.save(update_fields=["pending_comment"])
        ...
        self.ticket.refresh_from_db()
        self.assertEqual({}, self.ticket.pending_comment)
        self.assertEqual(2, len(self.ticket.comments))

    @patch("kitsune.customercare.tasks.sync_ticket_from_zendesk")
    @patch("kitsune.customercare.tasks.ZendeskClient")
    def test_verify_match_requires_created_at_after_lower_bound(self, mock_client_cls, mock_sync):
        self.ticket.pending_comment["attempts"] = 1
        self.ticket.pending_comment["last_attempted_at"] = "2026-05-14T01:00:00Z"
        self.ticket.save(update_fields=["pending_comment"])

        def fake_sync(ticket):
            # Same author + body but created BEFORE the lower bound — not a match.
            ticket.comments = [
                {
                    "id": 99,
                    "body": "hello",
    ...
    @patch("kitsune.customercare.tasks.sync_ticket_from_zendesk")
    @patch("kitsune.customercare.tasks.ZendeskClient")
    def test_first_attempt_skips_verify(self, mock_client_cls, mock_sync):
        # attempts == 0 (initial post). Verify should NOT run.
        mock_client_cls.return_value.add_ticket_comment.return_value = _build_audit(
            42, body="hello"
        )
    ...
    @patch("kitsune.customercare.tasks.ZendeskClient")
    def test_orphan_ticket_marks_failed_no_retries(self, mock_client_cls):
    ...
    def test_failed_pending_is_noop(self, mock_client_cls):
        ...
        self.ticket.pending_comment["status"] = "failed"
        self.ticket.pending_comment["allow_retries"] = True
        self.ticket.save(update_fields=["pending_comment"])
