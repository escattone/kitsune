...
class TicketReplyPostTests(TestCase):
    """POST to ticket_detail creates/replaces/re-attempts the pending_comment."""
    ...
    def test_same_body_resubmit_reattempts_existing(self, mock_task):
        ...
        self.assertEqual(1, pending["attempts"])  # not yet incremented; task does that
    ...
    @patch("kitsune.customercare.views.post_reply_to_zendesk")
    def test_same_body_when_allow_retries_false_is_noop(self, mock_task):
        """Failed + allow_retries=False + same body → silent no-op."""
        self.ticket.pending_comment = {
            "body": "hello",
            "status": "failed",
            "allow_retries": False,
    ...
    def test_post_strips_whitespace(self, mock_task):
        ...
        self.client.post(self._url(), data={"body": "  hello!  "})
        self.ticket.refresh_from_db()
        self.assertEqual("hello!", self.ticket.pending_comment["body"])
...
class TicketRepliesTemplateTests(TestCase):
    ...
    def _set_pending(self, **overrides):
        ...
        self.ticket.pending_comment = entry
        self.ticket.save(update_fields=["pending_comment"])
    ...
    def test_stale_sending_renders_as_failed(self):
        """effective_pending_comment coerces stale sending → failed-retryable."""
