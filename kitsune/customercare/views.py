...
def _replies_context(ticket, reply_form=None, sync_error=False):
    if reply_form is None:
        failed = ticket.effective_pending_comment
        initial = {"body": failed["body"]} if failed and failed.get("status") == "failed" else None
...
def ticket_detail(request, username, ticket_id):
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            existing = ticket.effective_pending_comment
            ...
            elif existing and new_body == existing["body"]:
                if not existing.get("allow_retries"):
                    # Same body that we know keeps failing; silent no-op. The
                    # error message in the form area explains why nothing happens.
                    return _render_replies(request, ticket, is_htmx, reply_form=form)
                # Re-attempt the existing pending. Task will increment attempts.
                ticket.pending_comment = {
                    **ticket.pending_comment,
                    "status": "sending",
                }
                ticket.pending_comment.pop("allow_retries", None)
